...
# === Configuration ===
# Most values are derived automatically or use defaults.
# You may need to adjust REGION, NAMES, or RESOURCE specs.

# AWS Region
AWS_REGION="eu-west-1" # Choose your desired AWS region (e.g., us-east-1, eu-west-1)

# ECR (Elastic Container Registry) Configuration
ECR_REPOSITORY_NAME="semsim-repo" # Choose a name for your ECR repository

# ECS (Elastic Container Service) Configuration
ECS_CLUSTER_NAME="semsim-cluster"   # Choose a name for your ECS cluster
ECS_SERVICE_NAME="semsim-service"   # Choose a name for your ECS service
ECS_TASK_FAMILY="semsim-task"     # Choose a family name for your ECS task definitions

# Application Configuration
CONTAINER_PORT=8080 # The port your application listens on inside the container
# Free Tier Eligible vCPU and Memory (Check AWS Free Tier documentation for current limits)
# Example: 0.25 vCPU, 0.5GB Memory
# https://aws.amazon.com/fargate/pricing/
TASK_CPU="256"    # CPU units (256 = 0.25 vCPU)
TASK_MEMORY="512" # Memory in MiB (512 = 0.5 GB)

# Local Docker Image Configuration
# The name and tag of the local Docker image built by build.sh
...
# Use list-clusters and grep to check for existence, as describe-clusters exit code was unreliable
...
echo "INFO: Registering ECS Task Definition '${ECS_TASK_FAMILY}'..."
# Define the task definition JSON. Note the 'arm64' architecture.
